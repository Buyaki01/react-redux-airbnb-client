...
const PhotosSection = ({addPhoto, setAddPhoto}) => {
  ...
  const submitPhotoByLink = async (e) => {
    e.preventDefault()
    const { data: filenames } = await addPhotoByLink({ photos: photoLink })
    setAddPhoto(prev => {
      ...
      <div id="uploadPhotos" className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {addPhoto.length > 0 && addPhoto.map(photoLink => (
            <div className="h-32 flex relative" key={photoLink}>
              <img className="rounded-2xl w-full object-cover" src={`${baseURL}/public/images/${photoLink}`} alt="photoLink" />
