...
const PhotosSection = ({addPhoto, setAddPhoto}) => {
  ...
  const submitPhotoByLink = async (e) => {
    e.preventDefault()
    const { data: filename } = await addPhotoByLink({ link: photoLink })
    setAddPhoto(prev => {
  ...
  return(
    <>
      ...
      <div aria-describedby="photos-description" className="flex gap-2 rounded-2xl">
        ...
        <button onClick={submitPhotoByLink} className="bg-gray-200 px-4 rounded-md">Add&nbsp;Photo</button>
      </div>
      ...
      <div id="uploadPhotos" className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {addPhoto.length > 0 && addPhoto.map(photoLink => (
            <div className="h-32 flex relative" key={photoLink}>
              <img className="rounded-2xl w-full object-cover" src={`https://airbnb-app-server.onrender.com/images/${photoLink}`} alt={photoLink} />
